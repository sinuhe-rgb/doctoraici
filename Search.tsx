    ...
    <div className="flex flex-col h-screen pb-20">
      ...
      <main className="flex-1 overflow-y-auto relative bg-slate-50 dark:bg-slate-900/20">
        ...
        {viewMode === 'list' ? (
          ...
          /* Map View */
          <div 
            className="relative w-full h-full overflow-hidden bg-slate-200 cursor-move touch-none"
          ...
          >
            ...
            <div 
                className="w-full h-full origin-center transition-transform duration-75 ease-linear will-change-transform"
                style={{ 
                    transform: `translate(${offset.x}px, ${offset.y}px) scale(${zoom})`,
                }}
            >
                <div className="absolute inset-0 bg-cover bg-center pointer-events-none" style={{backgroundImage: "url('https://lh3.googleusercontent.com/aida-public/AB6AXuBfEAv5RxFn0U6c8l0amwe_d-Jp_--mdEp4-LQBWgVPZS3nS0L_fn0YDIigUzk6j3PHxs2Ps-2JJa72ng3tLthErRVHqnJGjhnuRDymyrx_QZemY5f-L36TuO8kWt3fG9FdptVHnhytIAO2cnb71VKH3Am2TrkKNpFiONU07VtvI0nPVRMXQNjAJbzYKyMz_6cphJepE56FK1wDi0xWLlKmI2ZIRF-GHTQUieT3maOcHGihP4I8WpiZ6IQ-T4diOftXl9-x8OFAs-zz')"}}></div>
                ...
                <div 
                    ...
                    style={{ 
                        top: `${doctor.coordinates.y}%`, 
                        left: `${doctor.coordinates.x}%`,
                        // Counter-scale pins to keep them same visual size while zooming
                        transform: `translate(-50%, -50%) scale(${1 / zoom})` 
                    }}
                    onClick={(e) => {
                        e.stopPropagation(); // Stop drag event interaction
                        setSelectedMapDoctor(doctor);
                    }}
                    onMouseDown={(e) => e.stopPropagation()} // Prevent dragging when clicking a pin
                    onTouchStart={(e) => e.stopPropagation()}
                >
                    <div className={`p-2 rounded-full shadow-lg border-2 border-white transition-colors ${selectedMapDoctor?.id === doctor.id ? 'bg-primary text-white' : 'bg-primary/60 text-white hover:bg-primary'}`}>
             ...
             {/* Map Controls */}
            <div className="absolute right-4 bottom-56 flex flex-col gap-2 z-20" onMouseDown={e => e.stopPropagation()} onTouchStart={e => e.stopPropagation()}>
              <button 
                onClick={handleZoomIn}
                className="flex size-12 items-center justify-center rounded-xl bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 shadow-xl hover:bg-slate-50 transition-colors active:scale-95"
              >
              ...
              <button 
                onClick={handleReset}
                className="mt-2 flex size-12 items-center justify-center rounded-xl bg-primary text-white shadow-xl hover:bg-primary/90 transition-colors active:scale-95"
            ...
            {selectedMapDoctor && (
              <div className="absolute bottom-4 left-4 right-4 z-20 animate-in slide-in-from-bottom-5 duration-300" onMouseDown={e => e.stopPropagation()} onTouchStart={e => e.stopPropagation()}>
